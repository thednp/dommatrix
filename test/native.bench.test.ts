...
const cases: NativeCase[] = [
  ...
  { name: "setMatrixValue (transform list)", iterations: 2e4, shim: () => { const m = new CSSMatrix(); return () => m.setMatrixValue("translate(10px,20px)").toJSON() }, native: () => { const d = new DOMMatrix(); return () => d.setMatrixValue("translate(10px,20px)").toJSON() } },
  ...
  { name: "transformPoint", iterations: 1e5, shim: () => { const m = new CSSMatrix("rotate(30deg) translate(10px,20px)"); return () => m.transformPoint({ x: 1, y: 2, z: 3, w: 1 }) }, native: () => { const m = new DOMMatrix("rotate(30deg) translate(10px,20px)"); return () => { const p = m.transformPoint({ x: 1, y: 2, z: 3, w: 1 }); return { x: p.x, y: p.y, z: p.z, w: p.w } } } },
